...
const generateInvoicePDF = (bookingData, customerDetails, products) => {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(12).font('Helvetica')
      ...
      doc.text(`: ${customerDetails.state || 'N/A'}`, 50, 255)
...
exports.createDBooking = async (req, res) => {
  try {
    ...
    if (!customer_name || !company_name || !address || !district || !state || !products || !Array.isArray(products) || products.length === 0) {
      return res.status(400).json({ message: 'All required fields and products array are required' });
    }
    ...
    for (const product of products) {
      ...
      if (!id || !quantity || quantity < 1 || product_type !== 'gift_box_dealers') {
        return res.status(400).json({ message: 'Each product must have a valid ID, positive quantity, and product_type gift_box_dealers' });
      }
    ...
    const pdfPath = await generateInvoicePDF({ order_id, total }, customerDetails, products);
